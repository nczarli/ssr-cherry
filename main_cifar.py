...
parser.add_argument('--dataset_path', default='C:\\Users\\Nikodem\\Desktop\\Code\\SSR_BMVC2022\\datasets\\cifar-cherry', help='dataset path')
parser.add_argument('--noisy_dataset_path', default=None, help='open-set noise dataset path')
...
parser.add_argument('--noise_ratio', default=0.1, type=float, help='artifical noise ratio (default: 0.1)')
...
def train(labeled_trainloader1,iter_labeled_loader2, modified_label, all_trainloader, encoder, classifier, proj_head, pred_head, optimizer, epoch, args):
    ...
    for batch_idx, ([inputs_u1, inputs_u2], _, _, _) in enumerate(all_bar):
        try:
            ...
            [inputs_x1, inputs_x2], labels_x, _, index = next(iter_labeled_loader2)
...
def evaluate(dataloader, encoder, classifier, args, noisy_label, clean_label, i, stat_logs):
    ...
    with torch.no_grad():
        ...
        end_time = time.time()
        print(f'Feature extracting time: {end_time - start_time}')
...
def main():
    ...
    print('[INFO]: Dataset initialization')
    # start_time = time.time()
    ...
    # access the 1510th image in eval_data
    # print(eval_data[1509])
    # print(eval_data[1510])
    # print(eval_data[1511])
    ...
    # end_time = time.time()
    # print(f'[INFO]: Dataset initialization finished, time cost: {end_time - start_time:.2f}s')
    ...
    for i in range(args.epochs):
        ...
        # try create iter for eval_loader
        ...
        # torch.utils.data.dataloader.default_collate instead of torch.utils.data.dataloader.DataLoader

        # labeled_loader = torch.utils.data.default_collate(clean_subset, batch_size=args.batch_size, sampler=sampler, num_workers=0,pin_memory=True, drop_last=True)
    ...
    save_checkpoint({
        ...
        'pred_head': pred_head.state_dict(),
        'optimizer': optimizer.state_dict(),
    }, filename=f'{args.dataset}/{args.run_path}/last1.pth.tar')
